...
function moveSingleDirContents(dir) {
            ...
            return Q.nfcall(fs.rename, src, dst)
                .fail(function (err) {
                    if (err.code == 'ENOTEMPTY') {
                        sameNameErr = {};
                        sameNameErr.sourceName = dst;
                        sameNameErr.tempName = dst + '-tmp';

                        return Q.nfcall(fs.rename, src, sameNameErr.tempName);
                    } else {
                        throw err;
                    }
                });
    ...
    .then(function() {
        if (sameNameErr !== null) {
            return Q.nfcall(fs.rename, sameNameErr.tempName, sameNameErr.sourceName);
        }
    });
}
